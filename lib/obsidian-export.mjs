  ...
  const oralSection = oral
    ? [
      ...
      ...(oral.liveCorrections.length
        ...
        : []),
      ...(oral.liveCheckpoints?.length
        ? ["", "### Voice checkpoints", "", ...oral.liveCheckpoints.map((item) => [
          `- **${safeInline(item.dimension)}${item.targetEn ? ` · ${safeInline(item.targetEn)}` : ""}**: ${safeBlock(item.observationZh)}`,
          item.coachCueEn ? `  - Coach cue: ${safeInline(item.coachCueEn)}` : "",
          item.learnerRepeatEn ? `  - Learner repeat recorded in Chat: ${safeInline(item.learnerRepeatEn)}` : "",
