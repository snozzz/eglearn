...
const scoreV11 = z.object({
  status: z.enum(["assessed", "unassessed"]),
  band: z.number().int().min(1).max(5).nullable(),
  basis: z.enum(["transcript", "audio", "none"]),
...
const oralAnalysisV11 = z.object({
  evidenceMode: z.enum(["audio", "not_available"]),
...
export const reviewSchemaV11 = z.object({
  ...
  const audioAvailable = review.oralAnalysis.evidenceMode === "audio";
  if (!audioAvailable) {
  ...
  if (audioAvailable && review.pronunciation.status !== review.oralAnalysis.pronunciation.status) {
    context.addIssue({ code: "custom", message: "top-level pronunciation status must match oralAnalysis", path: ["pronunciation", "status"] });
  }
  if (audioAvailable && review.scores.fluency.status !== review.oralAnalysis.fluency.status) {
    context.addIssue({ code: "custom", message: "fluency score status must match oralAnalysis", path: ["scores", "fluency", "status"] });
  }
  if (review.scores.fluency.status === "assessed" && review.scores.fluency.basis !== "audio") {
    context.addIssue({ code: "custom", message: "assessed fluency requires audio basis", path: ["scores", "fluency"] });
  }
