...
// Deep v1.1 reviews cover more of a long sample, so a single strength may
// retain one additional learner quote without dropping useful evidence.
const strengthV11 = strength.extend({
...
export const reviewSchemaV11 = z.object({
  ...
  strengths: z.array(strengthV11).max(8),
...
function normalizeReviewInput(input) {
  if (
    ...
    !input.scores || typeof input.scores !== "object" || Array.isArray(input.scores) ||
    !input.scores.fluency || typeof input.scores.fluency !== "object" || Array.isArray(input.scores.fluency) ||
    input.scores.fluency.status !== "unassessed" ||
    input.scores.fluency.basis !== "not_available"
  ) {
    return input;
  }

  // Some earlier prompts copied oralAnalysis.evidenceMode into the score
  // basis. Canonicalize that transcript-only alias while retaining every
  // review field and quote.
  return {
    ...input,
    scores: {
      ...input.scores,
      fluency: { ...input.scores.fluency, basis: "none" },
    },
  };
}

function flattenUnionIssues(issues, parentPath = []) {
  return issues.flatMap((issue) => {
    const path = [...parentPath, ...(Array.isArray(issue.path) ? issue.path : [])];
    if (issue.code === "invalid_union" && Array.isArray(issue.errors)) {
      const nested = issue.errors.flatMap((branch) => flattenUnionIssues(branch, path));
      return nested.length ? nested : [{ ...issue, path }];
    }
    return [{ ...issue, path }];
  });
}
...
export function parseReviewText(text) {
  ...
  const schemaVersion = normalizedInput?.schemaVersion;
  const schema = schemaVersion === "1.0"
    ? reviewSchema
    : schemaVersion === "1.1"
      ? reviewSchemaV11
      : reviewInputSchema;
  ...
  return {
    success: false,
    errors: (schema === reviewInputSchema ? flattenUnionIssues(result.error.issues) : result.error.issues).map((issue) => {
