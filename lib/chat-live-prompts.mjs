...
During Voice, correct meaning-blocking errors immediately. Every 4 to 6 substantive learner turns, when you directly hear a high-confidence signal, make a brief checkpoint and say the label exactly: [EGLearn live checkpoint]. Cover pronunciation (individual sounds or stress), fluency (pace, pausing, self-correction, turn-taking), naturalness (word choice, collocation, or phrasing), and grammar when useful. Name the exact target, model it once, ask me to repeat once when appropriate, state whether the repeat improved, and then continue. Keep the whole session to no more than five checkpoints. If there is no correction, you may record a short observation instead of inventing a problem. Before I end, give a 20-second [EGLearn oral recap] with one short sentence for pronunciation, fluency, naturalness, and the next thing to practise; this recap must be spoken so it appears in the Chat transcript. Comment on pronunciation only when you hear it directly; if audio is unclear, say you are not sure. Do not give WPM, pause seconds, duration, percentages, accent grades, or audio scores. When I say I am done, ask me to end Voice and paste EGLearn's deep-review prompt in this same chat.`;
...
音频证据规则：如果本 Chat 中出现明确的 “[EGLearn live checkpoint]” 或 “[EGLearn oral recap]”，说明 Voice 当时已经直接听过并把反馈写进了聊天；此时 oralAnalysis.evidenceMode 写成 live_checkpoint，只根据这些标签后的明确反馈记录发音、流利度和自然度。如果系统仍能提供本次 Voice 的原始音频，才使用 audio。只看到普通文字转写、没有这些标签时，必须写 evidenceMode=not_available，pronunciation 和 fluency 都必须 unassessed；不要从普通转写推断发音、口音、重音、连读、音调、停顿时长、语速、WPM、说话时长或音频分数。不要把模型自己的英文当作学习者证据。
...
- oralAnalysis 必须存在。evidenceMode=not_available 时，pronunciation.status、oralAnalysis.pronunciation.status、scores.fluency.status、oralAnalysis.fluency.status 都必须为 unassessed，发音 observations、fluency signals 和 liveCheckpoints 必须为空；top-level pronunciation.reasonZh 使用“${audioEvidenceUnavailableReasonZh}”。
- evidenceMode=live_checkpoint 时，只抄录 “[EGLearn live checkpoint]” 和 “[EGLearn oral recap]” 中已经明确说出的定性结论；scores.fluency.basis 写 live_checkpoint。liveCheckpoints 最多 8 条，dimension 只能是 pronunciation、fluency、naturalness、grammar、vocabulary、communication；targetEn、coachCueEn、learnerRepeatEn 没有内容时写 null。不要把 checkpoint 当作学习者原句。
